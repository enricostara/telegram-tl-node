...
function TypeBuilder(module, tlSchema, isTypeFunction) {
    ...
    this._methods = [];
    this._type = isTypeFunction ? this.buildTypeFunction() : this.buildTypeConstructor();
}
...
TypeBuilder.prototype.buildTypeFunction = function() {
...
TypeBuilder.prototype._buildSerialize = function() {
    ...
    if (this.tlSchema.params) {
        for (var i = 0; i < this.tlSchema.params.length; i++) {
            ...
            var typeName = type[1];
            // Skip types with name starts with '!'
            ...
            if (typeName.charAt(0) === typeName.charAt(0).toUpperCase()) {
                body +=
                    ...
                    (('Object' === typeName || 'X' === typeName) ? '' : '.serialize()') + ');\n';
            }
...
TypeBuilder.prototype._buildDeserialize = function() {
    var body =
        '\tthis.deserialize = function deserialize (noLengthCheck) {\n' +
        '\t\tif (!this.constructor.super_.prototype.deserialize.call(this, arguments)) {\n' +
        '\t\t\treturn false;\n' +
        '\t\t}\n';
    // Parse the `TL-Schema params`
    if (this.tlSchema.params) {
        for (var i = 0; i < this.tlSchema.params.length; i++) {
            ...
            if (!type[3]) {
                ...
                // Manage Object type
                if (typeName.charAt(0) === typeName.charAt(0).toUpperCase()) {
                    if ('Object' === typeName || 'X' === typeName) {
                        body += '\t\tthis.' + param.name + ' = this._readBytes(this.bytes);\n';
                    } else {
                        body +=
                            '\t\tvar ' + typeName + ' = this.constructor.requireTypeByName(\'' + typeName + '\');\n' +
                            '\t\tvar obj = new ' + typeName + '({buffer: this._buffer, offset: this.getReadOffset()}).deserialize(noLengthCheck);\n' +
                            '\t\tif (obj) {\n' +
                            '\t\t\tthis.' + param.name + ' = obj;\n' +
            ...
            // Manage generic type
                ...
                body +=
                    ...
                    'buffer: this._buffer, offset: this.getReadOffset()}).deserialize(noLengthCheck);\n' +
                    '\t\tif (obj) {\n' +
                    '\t\t\tthis.' + param.name + ' = obj;\n' +
                    '\t\t\tthis._readOffset += obj.getReadOffset();\n' +
                    '\t\t}\n';
...
function buildTypes(schemas, types, targetModule, isTypeFunction) {
    for (var i = 0; i < schemas.length; i++) {
        var type = schemas[i];
        if (types.lastIndexOf(type[isTypeFunction ? 'method' : 'type']) >= 0) {
