...
TypeObject.prototype.deserialize = function (options) {
    ...
    if (!this._noId && !isBare) {
        ...
        if (this.id !== id) {
            var msg = 'Unable to de-serialize, (read id) ' + id + ' != (this.id) ' + this.id;
            logger.error(msg);
            throw new Error(msg);
        }
    }
...
// The method writes the `double` value given as argument
...
// The method reads a `double` value starting from the current position
