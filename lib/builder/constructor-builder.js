    ...
    if (this.tlSchema.params) {
        for (var i = 0; i < this.tlSchema.params.length; i++) {
            ...
            // Manage generic type
                ...
                if (typeParam !== 'long' && typeParam.slice(0, 3) !== 'int' && typeParam !== 'bytes' &&
                    typeParam !== 'String' && typeParam !== 'Object') {
                    typeParam = this.module + '.' + typeParam;
                }
                body +=
...
function requireTypeFromBuffer(buffer) {
    ...
    if (!type) {
        registryLogger.warn('Unable to retrieve a Type by ID %s', typeId);
        return;
    }
    if (registryLogger.isDebugEnabled()) {
...
function requireTypeByName(typeName) {
    ...
    if (!type) {
        registryLogger.warn('Unable to retrieve a Type by Name %s', typeName);
        return;
    }
    if (registryLogger.isDebugEnabled()) {
