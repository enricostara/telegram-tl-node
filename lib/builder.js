...
function Builder(module, tlSchema, messageWrapperType) {
    this.module = module;
    if (!this.module) {
...
Builder.prototype.buildTypeFunction = function (messageWrapperType) {
    ...
    // Create the function payload class re-calling Builder constructor.
    typeFunction.PayloadType = new Builder(this.module, this.tlSchema).getType();
...
function buildTypes(schemas, types, targetModule, isMethodType) {
    for (var i = 0; i < schemas.length; i++) {
        ...
        if (types.lastIndexOf(type[isMethodType ? 'method' : 'type']) >= 0) {
            ...
            var builder = new Builder('mtproto',  type, isMethodType);
